...
def position_side(ratio, avg_method, period):
    ...
    if avg_method == 'all':
        ...
        ratio_rolling_mean = ratio.shift(1).rolling(period, min_periods=0).mean()
        position_side_series = pd.Series(np.where(ratio>ratio_rolling_mean, 1, 
                                        np.where(ratio<ratio_rolling_mean, -1, np.nan)).tolist(), ratio.index)
    elif avg_method == 'ewm':
#         position_side_series = pd.Series(np.where(ratio>ratio.ewm(span=period).apply(lambda x : np.nanmean(x), raw=True), 1, -1).tolist(), ratio.index)
        ratio_ewm_mean = ratio.shift(1).ewm(span=period).mean()
        position_side_series = pd.Series(np.where(ratio>ratio_ewm_mean, 1, 
                                        np.where(ratio<ratio_ewm_mean, -1, np.nan)).tolist(), ratio.index)
...
#     asset_2_close = (asset_2_close - asset_2_close.mean()) / asset_2_close.std()
    ...
    long_return_series = ((1+real_signal)/2 * asset_1_pct +  (1-real_signal)/2 * asset_2_pct)
    long_short_return_series = (real_signal * (asset_1_pct - asset_2_pct))
    ...
    result = {
        ...
        "long_return_series":long_return_series,
        "long_short_return_series":long_short_return_series,
        "rsi":rsi,
...
def batch_pair_trading(code_list, data, method, period, rt_period, rsi_period, upper, lower, show_bar):
    """
    Implements:
    ...
    df_long_return_series = pd.DataFrame()
    df_long_short_return_series = pd.DataFrame()
    lens = len(code_list)
...
#             for j in tqdm_notebook(range(i+1, lens)):           
                ...
                code2 = code_list[j]
                try:
                    temp_result = single_pair_trading(data, code1, code2, method, period, rt_period, rsi_period,
                                                     upper, lower)
                    temp_result['ratio'].name = "%s-%s" % (code1,code2)
                    temp_result['long_asset'].name = "%s-%s" % (code1,code2)
                    temp_result['long_return_series'].name = "%s-%s" % (code1,code2)
                    temp_result['long_short_return_series'].name = "%s-%s" % (code1,code2)
                    ...
                    df_long_asset = pd.concat([df_long_asset, temp_result['long_asset']],
                                             axis=1)
                    df_long_return_series = pd.concat([df_long_return_series, temp_result['long_return_series']],
                                                      axis=1)
                    df_long_short_return_series = pd.concat([df_long_short_return_series, temp_result['long_short_return_series']],
                                                      axis=1)
    ...
    result['long_return_series'] = df_long_return_series
    result['long_short_return_series'] = df_long_short_return_series
